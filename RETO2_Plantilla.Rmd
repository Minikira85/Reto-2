...
```
#### Análisis descriptivo ejercicio 1:
La muestra incluyó 103.904 observaciones de pasajeros de vuelos comerciales, con datos completos salvo en retraso de llegada (0.3% faltantes). La variable Satisfaction presentó desbalance moderado (56.7% neutral o insatisfechos, 43.3% satisfechos).
...
modelo_step <- step(glm(Satisfaction ~ ., data = train, family = "binomial"), 
                    direction = "both", trace = FALSE)

test_completo <- test[complete.cases(test[, names(modelo_step$model)]), ]
prob_logit <- predict(modelo_step, newdata = test_completo, type = "response")
pred_logit <- factor(ifelse(prob_logit > 0.5, "satisfied", "neutral or dissatisfied"),
                     levels = levels(test_completo$Satisfaction))

tabla_logit <- table(Pred = pred_logit, Real = test_completo$Satisfaction)
tn_log <- tabla_logit[1,1]; fp_log <- tabla_logit[1,2]
fn_log <- tabla_logit[2,1]; tp_log <- tabla_logit[2,2]

exact_log <- mean(pred_logit == test_completo$Satisfaction)
...
test_lda <- test[complete.cases(test), ]
test_lda <- droplevels(test_lda)
...
# Asegurar que todas las categóricas son factor
...
nb_completo <- naiveBayes(Satisfaction ~ ., data = train_nb)
nb_pred_test <- predict(nb_completo, newdata = test_nb)
...
cat("Exactitud:", round(exact_log,3), 
    "Sens:", round(sens_log,3), 
    "Esp:", round(esp_log,3), 
    "Prec:", round(prec_log,3), 
    "F1:", round(f1_log,3), "\n\n")

cat("LDA:\n")
cat("Exactitud:", round(exact_lda,3), 
    "Sens:", round(sens_lda,3), 
    "Esp:", round(esp_lda,3), 
    "Prec:", round(prec_lda,3), 
    "F1:", round(f1_lda,3), "\n\n")

cat("Naive Bayes:\n")
cat("Exactitud:", round(exact_nb,3), 
    "Sens:", round(sens_nb,3), 
    "Esp:", round(esp_nb,3), 
    "Prec:", round(prec_nb,3), 
    "F1:", round(f1_nb,3), "\n\n")

kable(comparacion_modelos, digits=3, 
      caption = "**EJ6: COMPARACIÓN FINAL - 3 MODELOS (Test)**") %>%
  kable_styling(bootstrap_options = c("striped","hover","condensed"),
                full_width = FALSE) %>%
  row_spec(0, bold=TRUE, background="steelblue", color="white") %>%
  row_spec(which.max(comparacion_modelos$Exactitud),
